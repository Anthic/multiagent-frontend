      ...
      <div className="fixed top-8 right-8 z-[1001]">
        ...
        >
          ...
          <div className="relative w-4 h-4 flex items-center justify-center">
            <span className={`absolute h-[2px] w-full bg-black transition-all duration-300 ${isOpen ? 'rotate-45' : '-translate-y-1'}`} />
            <span className={`absolute h-[2px] w-full bg-black transition-all duration-300 ${isOpen ? '-rotate-45' : 'translate-y-1'}`} />
          </div>
        </button>
      </div>

      {/* Spreading Circular Panel */}
      <nav 
        className="fixed inset-0 bg-[#AAFFC7] z-[1000] flex flex-col justify-center items-center pointer-events-none"
        style={{
          clipPath: isOpen ? 'circle(150% at calc(100% - 65px) 50px)' : 'circle(0px at calc(100% - 65px) 50px)',
            ...
            <div key={link.name} className="overflow-hidden py-2">
              <TransitionLink 
                ...
                style={{
                  ...
                  opacity: isOpen ? 1 : 0,
                  transition: `transform 0.6s cubic-bezier(0.76, 0, 0.24, 1) ${isOpen ? 0.3 + (i * 0.08) : 0}s, opacity 0.6s ease ${isOpen ? 0.3 + (i * 0.08) : 0}s, text-shadow 0.3s ease`
                }}
              >
                {link.name}
              </TransitionLink>
